 ...
 * Wayland EGL backend.
 *
 * This backend pairs EGL/OpenGL ES with the Wayland compositor. It supports:
 *
 *  - GPU rendering, when an OpenGL ES based draw unit is enabled
 *    (LV_USE_DRAW_OPENGLES or LV_USE_DRAW_NANOVG). LVGL renders into the EGL
 *    window surface which is presented with eglSwapBuffers().
 *
 *  - Software rendering, when no GPU draw unit is enabled. The software
 *    framebuffer is uploaded to a texture and presented through EGL.
 *
 *  - An optional zero-copy software fast-path using the Linux DMA-BUF protocol
 *    (LV_WAYLAND_USE_EGL_DMABUF). Instead of uploading to a texture and
 *    rendering a full-screen quad through the EGL surface, the software
 *    framebuffer is uploaded directly into a DMA-BUF backed texture whose
 *    wl_buffer is attached to the surface. This avoids the extra quad pass and
 *    swap copy and is noticeably faster for software rendering on platforms
 *    that support it.
 *
 * The DMA-BUF fast-path is only relevant for software rendering, so it is
 * compiled in only when a GPU draw unit is *not* active. At run time the
 * backend probes for DMA-BUF support (compositor protocol, DRM/GBM device,
 * pixel format) and transparently falls back to the classic software path when
 * it is unavailable.
 */
...
static void init_buffer(lv_wayland_dmabuf_ctx_t * dmabuf_ctx, lv_wl_buffer_t * buffer, uint32_t width,
...
static void egl_flush_cb(lv_display_t * disp, const lv_area_t * area, uint8_t * px_map)
{
    ...
    /* Classic software path: upload framebuffer to a texture and present it
     * through the EGL window surface. */
    lv_opengles_viewport(0, 0, lv_display_get_original_horizontal_resolution(disp),
                         lv_display_get_original_vertical_resolution(disp));
