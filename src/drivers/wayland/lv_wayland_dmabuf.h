 ...
 *
 * Shared Linux DMA-BUF (zwp_linux_dmabuf_v1) protocol helper for the Wayland
 * backends. Both the G2D and the EGL backends share a buffer with the
 * compositor through this protocol; only the buffer *allocator* differs
 * (G2D uses lv_draw_buf/G2D memory, EGL uses gbm + an EGLImage). This module
 * owns everything that is independent of the allocator: binding the global,
 * the feedback/format negotiation, color-format conversion and the creation of
 * a wl_buffer from a DMA-BUF file descriptor.
 */
...
#if LV_USE_WAYLAND && LV_WAYLAND_USE_EGL_DMABUF
...
#endif /*LV_USE_WAYLAND && LV_WAYLAND_USE_EGL_DMABUF*/
