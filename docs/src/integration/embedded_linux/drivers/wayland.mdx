...
- <ApiLink name="LV_USE_OPENGLES" /> must be enabled in your LVGL configuration
...
#### Optional DMA-BUF Support

The Wayland EGL backend can optionally use the Linux DMA-BUF protocol for sharing
GPU buffers with the Wayland compositor.

DMA-BUF allows buffers allocated by the graphics driver to be passed directly to
the compositor, reducing copies and improving performance on supported systems.
This is especially useful on embedded Linux platforms where GPU/display
controllers support zero-copy or low-copy buffer sharing.

**Features:**

- Optional GPU buffer sharing through `linux-dmabuf`
- Potentially reduced memory copies
- Better performance on supported embedded platforms
- Useful together with OpenGL ES/EGL rendering
...
Depending on the platform and buffer allocation method, additional libraries such as `libdrm` or `gbm` may also be required.

**Usage:**

1. Enable OpenGL ES and DMA-BUF support in `lv_conf.h`:
...
2. Enable the Wayland driver in `lv_conf.h` with <ApiLink name="LV_USE_WAYLAND" /> set to `1`
3. <ApiLink name="LV_COLOR_DEPTH" /> should be set either to `32` or `16` in `lv_conf.h`
4. Add a display using <ApiLink name="lv_wayland_window_create" />,
...
To enable keyboard and encoder navigation, create a group and assign the relevant input devices to it:
...
lv_indev_t * enc = lv_wayland_get_indev_pointeraxis(disp);
lv_indev_set_group(kb,  g);
lv_indev_set_group(enc, g);
```

Once a group is set as default, any focusable widget created afterwards is automatically added to it,
allowing keyboard arrows and encoder rotation to move focus between widgets, and Enter/encoder click to activate them.

<Callout type="warning" title="Check display before querying input devices">
    <ApiLink name="lv_wayland_get_keyboard"/> and <ApiLink name="lv_wayland_get_pointeraxis"/> both return
...
To programmatically fullscreen the window, use the <ApiLink name="lv_wayland_window_set_fullscreen" />
function respectively with `true` or `false` for the `fullscreen` argument.

### Maximized mode

To programmatically maximize the window,
use the <ApiLink name="lv_wayland_window_set_maximized" /> function respectively with `true`
or `false` for the `maximized` argument.

### Minimize window

To programmatically minimize the window, use the <ApiLink name="lv_wayland_window_set_minimized" /> function.
...
Use <ApiLink name="lv_wayland_assign_physical_display" /> to assign a window to a particular physical
...
<ApiLink name="lv_wayland_unassign_physical_display" />:
...
You can create your own window decorations using LVGL widgets. For example, use the [Window (lv_win)](/widgets/win) widget to add a title bar with buttons,
